...
def satisfy(wI, G, Gp, params, s):
    ...
    for i in range(len(params)):
        for j in range(len(s)):
            distance[i, j] = numpy.abs(wI(s[j])) - \
                lI(Gp(G, params[i], s[j]), G(s[j]))
...
def setup_plot(legend_list, w1=False, w2=False, G=False, K=False, wr=False):
    ...
    if w1 and w2 and G:
        ...
        S = 1/(1+G*K)
        gain = numpy.abs(S(s))
        plt.loglog(wr*numpy.ones(2), [numpy.max(gain), numpy.min(gain)],
                   ls=':')
...
def setup_bode_plot(title_str, w=numpy.logspace(-2, 2, 100), func=False,
                    legend=False, plot=plt.loglog, grid=True):
